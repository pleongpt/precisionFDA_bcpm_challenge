...
Model for Phase 1 sub-challenge 2.

Input: DNA_CN expression matrix, Clinical data
Output: Predicted survival

Observations:
1. DNA_CN data has 811 attributes but only 174 samples.
2. Clinical data has asymmetric distributions in each attribute
   (sex, race, WHO grading, cancer type). Also lots of NaN.

Algorithm:
Among the 174 samples, separate them into training set and test set.
...
bPredictMode = False
default_trained_model_filename = 'p1sc2_sv_clf.joblib'
...
def load_input_data():
    # DNA_CN_gene_expresion data
    filepath = data_filepath('sc2_Phase1_CN_FeatureMatrix.tsv')
    if not os.path.isfile(filepath):
        filepath = alternative_data_filepath('sc2_Phase1_CN_FeatureMatrix.tsv')
        if not os.path.isfile(filepath):
            print("Data file sc2_Phase1_CN_FeatureMatrix.tsv is missing.")
            return None, None, None

    data_dna_CN = pd.read_csv(filepath, index_col='PATIENTID',
                           sep='\t', skipinitialspace=True)

    # Check the percentage of cells in data_dna_CN missing (NaN):
    data_dna_CN_missing_values_count = data_dna_CN.isnull().sum()
    data_dna_CN_total_cells = np.product(data_dna_CN.shape)
    total_missing = data_dna_CN_missing_values_count.sum()

    print("Shape of DNA_CN data: %s, max=%.4f, min=%.4f, NaN=%.2f%%" % (data_dna_CN.shape, max(data_dna_CN.max()),
                                                                     min(data_dna_CN.min()),
                                                                     (total_missing/data_dna_CN_total_cells) * 100))

    # Clinical Phenotypes (SEX, RACE, WHO_GRADING, CANCER_TYPE)
    filepath = data_filepath('sc2_Phase1_CN_Phenotype.tsv')
    if not os.path.isfile(filepath):
        filepath = alternative_data_filepath('sc2_Phase1_CN_Phenotype.tsv')
        if not os.path.isfile(filepath):
            print("Data file sc2_Phase1_CN_Phenotype.tsv is missing.")
            return None, None, None
    ...
    filepath = data_filepath('sc2_Phase1_CN_Outcome.tsv')
    if not os.path.isfile(filepath):
        filepath = alternative_data_filepath('sc2_Phase1_CN_Outcome.tsv')
        if not os.path.isfile(filepath):
            print("Data file sc2_Phase1_CN_Outcome.tsv is missing.")
            return None, None, None
    ...
    flag = len(set(list(data_dna_CN.index.values))) == len(list(data_dna_CN.index.values))
    if flag:
        print("All PATIENTID entries in data_dna_CN are unique.")
    else:
        print("Warning: Some PATIENTID entries in data_dna_CN are duplicates!")
    ...
    if (list(data_dna_CN.index.values).sort() == list(data_phenotype.index.values).sort()):
        print("PATIENTID entries in data_dna_CN are the same as in data_phenotype.")
    else:
        print("Warning: PATIENTID entries in data_dna_CN are NOT the same as in data_phenotype!")

    if (list(data_dna_CN.index.values).sort() == list(data_survival.index.values).sort()):
        print("PATIENTID entries in data_dna_CN are the same as in data_survival.")
    else:
        print("Warning: PATIENTID entries in data_dna_CN are NOT the same as in data_survival!")

    print()

    return data_dna_CN, data_phenotype, data_survival
...
def show_most_important_attributes(classifier_pipeline, clf_name, attribute_names, topN):
    if clf_name == "log_reg_l1" or clf_name == "elastic_net":
        ...
        topN_filename = 'p1sc2_topN_'+clf_name+'.txt'
...
def model_DNA_CN_and_phenotype_vs_survival_data(data_dna_CN, data_phenotype, data_survival, bPhenotype):
    # Algorithm:
    # 1. First make sure the entries in data_dna_CN and in data_survival
    #    are aligned.
    # 2. We know that data_dna_CN and data_survival has no NaN entry from
    #    script p1_sc1_inspect_data.py.
    # 3. We normalize the values in data_dna_CN to avoid having any outlier from
    #    skewing the data.
    ...
    #    Since the number of attributes of data_dna_CN is huge, we should attempt
    ...
    numeric_features_names = list(data_dna_CN.columns)
    ...
    numeric_features_indices = [ i for i in range(data_dna_CN.shape[1])]
    ...
    if bPhenotype:
        ...
        # Reindex data_grading_cancer_type such that it aligns with the index in data_dna_CN
        data_grading_cancer_type = data_grading_cancer_type.reindex(data_dna_CN.index)
        data_input = pd.concat([data_dna_CN, data_grading_cancer_type], axis=1)
        ...
        categorical_features_indices = [i+data_dna_CN.shape[1] for i in range(data_grading_cancer_type.shape[1])]
    else:
        data_input = data_dna_CN
        categorical_features_names = []
        categorical_features_indices = []

    # Reindex data_survival such that it aligns with the index in data_dna_CN
    # (They should be the same in the raw data but just in case.)
    data_survival = data_survival.reindex(data_dna_CN.index)
    ...
    # dna_CN_predict = data_dna_CN.iloc[test_index]
    # filepath = data_filepath('dna_CN_predict.tsv')
    # directory = os.path.dirname(filepath)
    # if not os.path.exists(directory):
    #     os.makedirs(directory)
    # dna_CN_predict.to_csv(filepath, sep='\t', index=True)
    ...
    # The "l1" penalty helps to remove some of the irrelevant DNA_CN attributes
    ...
    if model_flags & FLAG_LOG_REG:
        ...
        print()
        print("**************************************************************************")
        print("Logistic Regression with L1 regularization on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_log_reg_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_log_reg_clf.joblib')
    ...
    if model_flags & FLAG_ELASTIC:
        ...
        print()
        print("**************************************************************************")
        print("Elastic Net Regression on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_elastic_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_elastic_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
    ...
    if model_flags & FLAG_LINEAR_SVM:
        ...
        print()
        print("**************************************************************************")
        print("SVM with linear kernel on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_linSVM_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_linSVM_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
    ...
    if model_flags & FLAG_GAUSSIAN_RBF_SVM:
        ...
        print()
        print("**************************************************************************")
        print("SVM with Gaussian RBF kernel on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_rbfSVM_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_rbfSVM_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
    ...
    if model_flags & FLAG_RF:
        ...
        print()
        print("**************************************************************************")
        print("Random Forest Classifier on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_rf_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_rf_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
    ...
    if model_flags & FLAG_XGBOOST:
        print()
        print("**************************************************************************")
        print("XGBoost Classifier on DNA_CN_data vs survival labels:")
        ...
        if not model_flags & FLAG_SOFT_VOTING:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_xgboost_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_xgboost_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
    ...
    if model_flags & FLAG_SOFT_VOTING:
        print()
        print("**************************************************************************")
        print("Soft Voting Classifier on DNA_CN_data vs survival labels:")
        print("**************************************************************************")
        estimators = []
        if model_flags & FLAG_LOG_REG:
        ...
        if len(estimators) > 0:
            ...
            # Save model to file
            print()
            print("Save model to file ./data/%s" % 'p1sc2_sv_clf.joblib')
            print()
            model_filepath = data_filepath('p1sc2_sv_clf.joblib')
            directory = os.path.dirname(model_filepath)
            if not os.path.exists(directory):
                os.makedirs(directory)
            if os.path.isfile(model_filepath):
...
def predict_based_on_model(dna_CN_predict_filepath, phenotype_predict_filepath, trained_model_filepath):
    # clf_pipeline is the trained model
    if os.path.isfile(trained_model_filepath):
        clf_pipeline = load(trained_model_filepath)

    dna_CN_predict = pd.read_csv(dna_CN_predict_filepath, index_col='PATIENTID',
                           sep='\t', skipinitialspace=True)
    ...
    # Reindex data_grading_cancer_type such that it aligns with the index in dna_CN_predict
    data_grading_cancer_type = data_grading_cancer_type.reindex(dna_CN_predict.index)
    test_data = pd.concat([dna_CN_predict, data_grading_cancer_type], axis=1)
    ...
    filepath = os.path.join(PROJECT_ROOT_DIR, "outputs", 'subchallenge_2.tsv')
...
def main(_):
    ...
    if bPredictMode:
        if FLAGS.dna_CN_predict is None:
            print("Must specify DNA_CN filename for making a prediction.")
            return
        ...
        dna_CN_predict_filepath = FLAGS.dna_CN_predict.strip('\"')
        phenotype_predict_filepath = FLAGS.pheno_predict.strip('\"')

        if FLAGS.model == "log_reg":
            trained_model_filepath = data_filepath('p1sc2_log_reg_clf.joblib')
        elif FLAGS.model == "elastic":
            trained_model_filepath = data_filepath('p1sc2_elastic_clf.joblib')
        elif FLAGS.model == "lin_SVM":
            trained_model_filepath = data_filepath('p1sc2_linSVM_clf.joblib')
        elif FLAGS.model == "rbf_SVM":
            trained_model_filepath = data_filepath('p1sc2_rbfSVM_clf.joblib')
        elif FLAGS.model == "RF":
            trained_model_filepath = data_filepath('p1sc2_rf_clf.joblib')
        elif FLAGS.model == "xgboost":
            trained_model_filepath = data_filepath('p1sc2_xgboost_clf.joblib')
        elif FLAGS.model == "soft_voting":
            trained_model_filepath = data_filepath('p1sc2_sv_clf.joblib')
        else:
        ...
        predict_based_on_model(dna_CN_predict_filepath, phenotype_predict_filepath, trained_model_filepath)
        return
    ...
    data_dna_CN, data_phenotype, data_survival = load_input_data()
    if (data_dna_CN is None) or (data_phenotype is None) or (data_survival is None):
        # Quit the program
        return
    ...
    model_DNA_CN_and_phenotype_vs_survival_data(data_dna_CN, data_phenotype, data_survival, bAnalyze_Phenotype)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-dna_CN_predict',
        type=str,
        help='Specify the input DNA CN filename for generating a prediction.'
    )
    parser.add_argument(
        '-pheno_predict',
        type=str,
